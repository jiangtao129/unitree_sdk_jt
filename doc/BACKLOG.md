...
| P1-A09 | TeeBuf class 加 `final` 关键字(防止意外被继承) | `keyDemo3.cpp` | [ ] | — |
...
| # | 描述 | 文件 | 状态 | PR |
|---|---|---|---|---|
| P1-C01 | actions/checkout@v4 → @v5(消除 Node 20 deprecation 警告) | `.github/workflows/c-cpp.yml` | [ ] | — |
| P1-C02 | CI 加 `actions/cache@v4` 缓存 cmake build 产物(下次 PR 提速) | `.github/workflows/c-cpp.yml` | [ ] | — |
| P1-C03 | CI 加 `timeout-minutes: 15`(防止僵死 job 永远占资源) | `.github/workflows/c-cpp.yml` | [ ] | — |
| P1-C04 | CI 加 `concurrency` group(同一 PR 重复 push 取消旧 run) | `.github/workflows/c-cpp.yml` | [ ] | — |
| P1-C05 | CI 加 `permissions: contents: read`(最小权限原则) | `.github/workflows/c-cpp.yml` | [ ] | — |
...
| # | 描述 | 文件 | 状态 | PR |
|---|---|---|---|---|
| P2-D01 | 加 `CHANGELOG.md`,记录 PR #1~ 至今的语义化版本时间线 | 新文件 | [ ] | — |
| P2-D02 | 加 `CONTRIBUTING.md`,说明 fork 接受新 PR 的流程 | 新文件 | [ ] | — |
| P2-D03 | 加 `SECURITY.md`,说明发现安全问题怎么报告 | 新文件 | [ ] | — |
| P2-D04 | 加 `.editorconfig`(行尾 / 缩进 / 字符集统一) | 新文件 | [ ] | — |
...
| P2-D14 | `slam_interfaces_server_config/param.yaml` 加中文注释 | `unitree_slam/config/slam_interfaces_server_config/param.yaml` | [ ] | — |
| P2-D15 | `planner_config/{param,pose_ctrl_param}.yaml` 加中文注释 | `unitree_slam/config/planner_config/*.yaml` | [ ] | — |
